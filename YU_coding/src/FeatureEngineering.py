...
class FeatureEncoder:
    ...
    def fit_transform(self, X: pd.DataFrame, y: pd.Series) -> pd.DataFrame:
        ...
        for col, cfg in self.feature_config.items():
            ...
            #hinzugefuergt fuer native_scaling
            elif encoding == "CATEGORY_KEEP":
                encoded_parts.append(X[[col]].copy())
        ...
        #incert for native_Scaling
        for col, cfg in self.feature_config.items():
            if cfg["encoding"] == "CATEGORY_KEEP" and col in X_encoded.columns:
